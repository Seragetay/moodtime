...
def read_file():
    ...
    lis_filter = ["Unknown", "Y", "N"] # low pop
    #w_df = w_df[w_df["Org_Disability"].isin(["N"])]
...
def single_q(question): # for cat with single question
    ...
    pop = len(valid_col)
    value = round(((value - 0) * 100)/(10-0), 2)
...
def care_and_cordnate(question, denm): # for cat with single question
    ...
    value = valid_col[question].mean()
    pop = len(valid_col)
    value = round(((value - 1) * 100)/(denm-1), 2)
...
health_Plan_Customer_Service_total_count = overAllpop_3("q34", "q35", "Q36_37")
...
getting_Needed_Prescription_Drugs_total = overAllpop_4("q42", "q44", "q42_q44", "q40")
print(getting_Needed_Prescription_Drugs_total)
...
care_Coordinationtotal_total_score = overAllpop_5("q20_q21", "q18", "q23", "q26", "q32")
print(care_Coordinationtotal_total_score)
